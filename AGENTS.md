...
- Everything under `src/lib/data/` (events, players, decks, archetypes, seasons,
  card images/colors/types) is generated by the Python pipeline in `scripts/`
  from raw data in `data/` - never hand-edit those files directly. See
  `UPDATING_THE_WEBSITE.md` for the full pipeline and how to run it after a
  new event.
